...
export class CommunitySwitch implements m.ClassComponent<Attrs> {
    ...
    view({attrs}: m.CVnode<Attrs>) {
        return m('.community-switch', 
          this.communities.length > 1 ?
          m('.dropdown', [
            m(
              'button.btn.btn-md.btn-secondary.dropdown-toggle', 
              { 'data-bs-toggle': 'dropdown', type: 'button', 'aria-expanded': 'false', 'aria-haspopup': 'menu' }, 
              ...
              this.communities.map((communityId) => 
                m('li', { role: 'presentation' }, 
                  m(
                    ...
                    { type: 'button', role: 'menuitem', onclick: () => this.updateCommunityId(communityId, attrs) }, 
                    communityId
                  )
                )
              )
          ...
          m('.mb-4.mt-2',
            m('.row', 
              m('.col.fw-bold', this.communityId)
            )
          )
        )
    }
