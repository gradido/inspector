...
export class Pagination implements m.ClassComponent<Attrs> {
  ...
  lastVisibleButton(currentPage: number, totalPages: number): number {
    return Math.min(
      totalPages, 
      this.visibleNumberButtons(currentPage, totalPages) + this.firstVisibleButton(currentPage, totalPages)
    )
  }
  ...
  makeEllipsisButton(): m.Vnode {
    return m('li.page-item.disabled.bv-d-sm-down-none', { role: 'separator' },
      m('span.page-link', '...')
    )
  }
  ...
  makeArrowButton(disabled: boolean, ariaLabel: string, buttonLabel: string, targetPage: number, attrs: Attrs): m.Vnode {
  ...
  view({attrs}: m.CVnode<Attrs>) {
    ...
    return m('nav.mt-3', { 
      ...
      m('ul.justify-content-center.pagination.pagination-lg', { class: attrs.pill ? 'b-pagination-pills' : '' }, [
        this.makeArrowButton(isFirst, t.__('Go to first page'), '«', 1, attrs),
        this.makeArrowButton(isFirst, t.__('Go to previous page'), '‹', attrs.currentPage - 1, attrs),
        this.pages(attrs),
        this.makeArrowButton(isLast, t.__('Go to next page'), '›', attrs.currentPage + 1, attrs),
