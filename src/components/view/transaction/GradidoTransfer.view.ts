...
export class GradidoTransferView implements m.ClassComponent<ViewAttrs> {
  viewDetails(attrs: ViewAttrs, crossGroupType: CrossGroupType) {
    ...
    if (!transfer) {
      throw new Error(`invalid transaction, expect transfer, but get: ${JSON.stringify(attrs)}`)
    }
  ...
  thirdRowView(crossGroupType: CrossGroupType, amount: string) {
  ...
  view({ attrs }: m.CVnode<ViewAttrs>) {
    ...
    return m(DetailsBlock, {
      ...
      thirdRow: this.thirdRowView(
        crossGroupType,
        getAmount(attrs.transaction.gradidoTransaction.bodyBytes),
      ),
