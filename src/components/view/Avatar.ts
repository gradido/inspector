...
import { getInitials, getUsername } from '../../models/linkedUser'
    ...
    return m(
      '.app-avatar.d-flex.justify-content-center.align-items-center.rounded-circle',
      {
        style: {
          width: '42px',
          height: '42px',
          'background-color': this.lightColor(this.asciiValue(user)),
          'text-transform': 'uppercase',
        },
      },
      m(
        'span.font-medium',
        { style: { 'font-size': '16.8px', 'line-height': 1, color: 'white' } },
        getInitials(user),
      ),
    )
  }
