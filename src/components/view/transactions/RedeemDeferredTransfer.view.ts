...
export class RedeemDeferredTransferView implements m.ClassComponent<ViewAttrs> {
  viewDetails(attrs: ViewAttrs) {
    const redeemDeferredTransfer =
      attrs.transaction.gradidoTransaction.bodyBytes.redeemDeferredTransfer
    if (!redeemDeferredTransfer) {
      throw new Error(
        `invalid transaction, expect redeemDeferredTransfer, but get: ${JSON.stringify(attrs)}`,
      )
    }
