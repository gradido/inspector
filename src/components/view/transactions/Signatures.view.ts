    ...
    return [
      ...
      attrs.signaturePairs.map(signaturePair => [
        m('.row', [
          ...
          m('.col', m(CopyToClipboardLink, { data: signaturePair.pubkey, name: t.__('Public Key') })),
        ]),
        m('.row', [
          m('.col', t.__('Signature')),
          m('.col', m(CopyToClipboardLink, { data: signaturePair.signature, name: t.__('Signature'), maxLength: 64 })),
        ]),
        m('.row.pb-2')
      ])
    ]
