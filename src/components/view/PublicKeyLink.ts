...
export class PublicKeyLink implements m.ClassComponent<Attrs> {
  view({ attrs }: m.CVnode<Attrs>) {
    ...
    if (attrs.maxLength && attrs.publicKey.length > attrs.maxLength - 3) {
      ...
      publicKey =
        attrs.publicKey.substring(0, partLength) +
        '...' +
        attrs.publicKey.substring(attrs.publicKey.length - partLength)
    }
    return m(
      'a',
      {
        href: `${m.route.prefix}/account/${attrs.communityId}/${attrs.publicKey}`,
        title: attrs.publicKey,
      },
      publicKey,
    )
  }
