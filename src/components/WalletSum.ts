...
export class WalletSum implements m.ClassComponent<Attrs> {
  ...
  view({ attrs }: m.CVnode<Attrs>) {
    ...
    return m(
      attrs.active ? '.active.router-link-exact-active' : '',
      { 'aria-current': attrs.active ? 'page' : undefined },
      m('.translucent-color-opacity', [
        m('.text-center', m(combineElementWithClasses('span', badgeClasses), attrs.name)),
        m(
          '.bg-white.app-box-shadow.gradido-border-radius.p-4.gradido-global-border-color-accent',
          ...
          [
            ...
            m('.row', [
              m('.col-9', { style: { height: '26px' } }, [
                m(
                  'svg.me-3.gradido-global-border-color-accent.gradido-global-color-accent.d-none.d-lg-inline',
              ...
              m(
                '.col-3.border-start.border-dark',
                m(
